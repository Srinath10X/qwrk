...
- **Effects created outside a derive outlive their component.** `effect()` and `.effect()` created in a component that no derive renders keep running until you stop them. Inside a derive they stop when it runs again.
...
`effect()` and `.effect()` created outside a derive are the exception: they live until you stop them, and they keep the states they read alive. Those created while a derive runs, such as in a list item, stop when it runs again, and are freed with it when its DOM is dropped.
